...
function insertPokes(req, res) {
  for (let i = 1; i <= 150; i++) {
    request.get('https://pokeapi.co/api/v2/pokemon/'+i, (error, res, body) => {
      ...
      pokemon.save((error, res) => {
        if (error) return console.err('Error en guardar pokemon', error.message)
      })
    })
  }
...
function savePoke(req, res) {
  ...
  poke.save((err, pokeStored) => {
    if (err) return res.status(500).send({ message: `Error al salvar el pokemon ${err}`})
